...
cd "${script_path}/../" && ./build.sh ${debug} update make utils std

# create a repo using the resulting packages
mkdir -p ${repo_dir}
if [[ -n "${git}" ]]; then
    cp "${script_path}/../packages/_utils/zfs-utils-git/"*.pkg.tar.xz ${repo_dir}
    cp "${script_path}/../packages/linux/zfs-linux-git/"*.pkg.tar.xz ${repo_dir}
else
    cp "${script_path}/../packages/_utils/zfs-utils/"*.pkg.tar.xz ${repo_dir}
    cp "${script_path}/../packages/linux/zfs-linux/"*.pkg.tar.xz ${repo_dir}
fi
